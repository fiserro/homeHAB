...
#
# This table maps PWM duty cycle % to measured output voltage.
...
# 2. Set OpenHAB to 10%, 20%, ... 100% and measure voltage
# 3. Write measured voltages here
# 4. Re-deploy
#
...
# Switch between calibrated and linear mode:
# - Use PWM_CALIBRATION for normal operation (calibrated)
# - Use PWM_CALIBRATION_LINEAR for measuring raw PWM output
ACTIVE_CALIBRATION = PWM_CALIBRATION
# ACTIVE_CALIBRATION = PWM_CALIBRATION_LINEAR  # uncomment for measuring


def pwm_for_voltage(target_voltage: float) -> float:
    """
    ...
    points = sorted(ACTIVE_CALIBRATION.items(), key=lambda x: x[0])
...
def percent_to_pwm(percent: float) -> float:
    """
    Convert desired output percent (0-100%) to calibrated PWM duty cycle.
    ...
    return pwm_for_voltage(target_voltage)
