...
Runs as Docker container on RPi (`privileged` mode for GPIO/SPI access). Deployed via:

```bash
./scripts/deploy-docker.sh prod
```

Configuration is in `docker-compose.yml` (service `hrv-bridge`, profile `prod`).
...
```bash
ssh openhab.home 'docker logs -f homehab-hrv-bridge'
ssh openhab.home 'docker restart homehab-hrv-bridge'
```
